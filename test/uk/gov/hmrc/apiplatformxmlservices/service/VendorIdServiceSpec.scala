...
class VendorIdServiceSpec extends AnyWordSpec with Matchers with MockitoSugar with BeforeAndAfterEach {
  ...
  trait Setup {
    ...
    val organisationWithStartingVendorId = Organisation(organisationId = OrganisationId(uuid), vendorId = configStartingVendorId, name = "Organisation Name")
    val organisationWithVendorId4001 = Organisation(organisationId = OrganisationId(uuid), vendorId = vendorId4001, name = "Organisation Name")
  ...
  "getNextVendorId" should {
    "return (maxVendorId + 1) when max vendorId from repo is greater than or equal to config startingVendorId" in new Setup {
      ...
      result shouldBe Some(vendorId9001)
    ...
    "return config startingVendorId when max vendorId from repo is less than config startingVendorId" in new Setup {
      ...
      val result = await(inTest.getNextVendorId)
      result shouldBe Some(configStartingVendorId)
    ...
    "return None when repo returns Exception" in new Setup {
      when(mockOrganisationRepo.findOrgWithMaxVendorId).thenReturn(Future.failed(new Exception("error")))
